...
def analysis_result_central(num_robots=4, reach_radius=0.76, result_folder_path = "analysis/"):
    result_files = []
    for _, _, filenames in os.walk(result_folder_path):
        for filename in filenames:
            if filename.endswith(".npz"):
    ...
    model = torch.load("GNN/trained_nn.pkl", map_location=device)

    for result_file in result_files:
        # result_file = "map0016robots15_test_result.npz" # for test
        data = np.load(result_folder_path + result_file)
        # 先判断是否存在ros代理连上的情况 #在目标点与起始点位置很近的情况，可能存在问题。   需要判断每个机器人的代理是否连接上
        input = torch.tensor(data["decision_pcpts"]).float().to(device)
        output = torch.tensor(data["decisions"]).float().to(device)
        adj_list = [matrix2table(matrix) for matrix in data["decision_adj"].tolist()]
        index = [0 for i in data["decisions"]]
        test_ouput = model(input, adj_list, index)
        adj = [matrix2table(np.zeros([4,4])) for i in data["decisions"]]
        single_output = model(input, adj, index)
        print(result_file)
        print("record data output : {}".format(test_ouput))
        print("personal data output : {}".format(single_output))
        print("decision position : {}".format(data["decision_pos"]))
        print("decision_output : {}".format(data["decision_output"]))
        print("decisions : {}".format(data["decisions"]))
