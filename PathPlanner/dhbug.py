...
class DhPathPlanner:
    ...
    def is_forward_safe(self):
        """判断当前正前方是否存在适合行走的安全扇面"""
        self.get_robot_state()  # 更新机器人当前位置、朝向角、速度及激光传感器检测到的障碍物信息

        safe_intervals = self.find_safe_angle_intervals()
        head_index_start = self.find_angle_index(self.laser_angles, -1*self._sector_safe_width/2)
        head_index_end = self.find_angle_index(self.laser_angles,self._sector_safe_width/2)
        for interal in safe_intervals:
            if head_index_start >= interal[0] and interal[1] >= head_index_end:
                return True
        return False
